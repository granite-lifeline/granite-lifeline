...
from report_quality_evaluator import (  # noqa: E402
    evaluate_report,
...
def extract_reports_from_markdown(md_path: Path) -> list:
    ...
    - report (dict with generated report sections)
    """
    ...
    for match in matches:
        ...
        if action_items:
            ...
            lines = [
                line.strip()
                for line in recommended_action.split('\n')
                if line.strip()
            ]
            actions = [
                line.lstrip('- ').strip()
...
def main():
    ...
    print()
    print(
        f"{'Scenario':<30} {'Baseline':<12} "
        f"{'RAG':<12} {'Improvement':<12}"
    )
    print("-" * 70)
    ...
    for baseline, rag in zip(baseline_scores, rag_scores):
        ...
        print(
            f"{baseline.anomaly_type:<30} "
            f"{baseline.overall_score:<12.2f} "
            f"{rag.overall_score:<12.2f} "
            f"{improvement_str:<12}"
        )

    # Calculate averages
    baseline_avg = (
        sum(s.overall_score for s in baseline_scores) / len(baseline_scores)
    )
    ...
    print("-" * 70)
    print(
        f"{'AVERAGE':<30} {baseline_avg:<12.2f} "
        f"{rag_avg:<12.2f} {avg_improvement:+.2f}"
    )
    print()
