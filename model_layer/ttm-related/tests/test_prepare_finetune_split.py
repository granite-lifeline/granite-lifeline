"""
Story 6 tests: per-segment eligibility (length + data-quality gate)
and trip-grouped train/validation split on Group 1's schema v1 data
...
def make_key_frame(
    segments: dict, unhealthy_rows: dict | None = None
) -> pd.DataFrame:
    """Key-columns frame: {segment_id: (trip_id, rows)}.

    All rows default to a passing data-quality gate
    (`condition_quality_flags == "OK"`, `condition_confidence ==
    "high"`). ``unhealthy_rows`` = {segment_id: [row_in_segment, ...]}
    flips those specific rows to a failing quality flag.
    """
    frames = [
        pd.DataFrame(
            {
    ...
    combined[QUALITY_FLAGS_COLUMN] = "OK"
    combined[CONFIDENCE_COLUMN] = "high"
    for segment, rows in (unhealthy_rows or {}).items():
        mask = (combined["segment_id"] == segment) & (
            combined["row_in_segment"].isin(rows)
        )
        combined.loc[mask, QUALITY_FLAGS_COLUMN] = "MISSING_MAF"
        combined.loc[mask, CONFIDENCE_COLUMN] = "medium"
    return combined


class TestRepoRelative:
    def test_path_under_repo_root_is_relativized(self):
        path = (
            _REPO_ROOT / "data_layer" / "production_feature_manifest"
            / "production_features.csv"
        )
        assert repo_relative(path) == (
            "data_layer/production_feature_manifest/"
            "production_features.csv"
        )

    def test_path_outside_repo_kept_as_given(self, tmp_path):
        path = tmp_path / "x.csv"
        assert repo_relative(path) == str(path)
...
class TestUnhealthySegments:
    def test_all_healthy_gives_empty_dict(self):
        ...
        assert unhealthy_segments(features) == {}
    ...
    def test_flagged_rows_counted_per_segment(self):
        features = make_key_frame(
            ...
            unhealthy_rows={"trip_0002_seg_001": [2, 4]},
        )
        assert unhealthy_segments(features) == {
            "trip_0002_seg_001": 2
        }

    def test_low_confidence_alone_counts_as_unhealthy(self):
        features = make_key_frame(
            {"trip_0001_seg_001": ("trip_0001", 5)}
        )
        # Quality flag OK but confidence degraded: still fails the
        # gate (defense-in-depth, either column alone can flag).
        features.loc[2, CONFIDENCE_COLUMN] = "low"
        assert unhealthy_segments(features) == {
            "trip_0001_seg_001": 1
        }

    def test_bad_quality_flag_alone_counts_as_unhealthy(self):
        features = make_key_frame(
            {"trip_0001_seg_001": ("trip_0001", 5)}
        )
        features.loc[3, QUALITY_FLAGS_COLUMN] = "MISSING_RPM"
        assert unhealthy_segments(features) == {
            "trip_0001_seg_001": 1
        }
...
class TestBuildManifest:
    ...
    def make_multi_trip_inputs():
        ...
        return make_key_frame(
            segments,
            unhealthy_rows={"trip_0002_seg_003": [1]},
        )

    def test_manifest_schema_and_values(self):
        features = self.make_multi_trip_inputs()
        ...
        assert manifest["quality_gate"] == {
            QUALITY_FLAGS_COLUMN: "OK",
            CONFIDENCE_COLUMN: "high",
        }
    ...
    def test_trip_grouped_split_invariant(self):
        features = self.make_multi_trip_inputs()
        manifest = build_manifest(
            features, "input.csv", min_rows=10,
        )
    ...
    def test_no_eligible_segments_raises(self):
        ...
        with pytest.raises(ValueError, match="[Nn]o eligible trips"):
            build_manifest(features, "input.csv", min_rows=700)
...
class TestLoaders:
    def test_load_key_columns_returns_keys_only(self, tmp_path):
        ...
        assert set(frame.columns) == set(
            KEY_COLUMNS + [QUALITY_FLAGS_COLUMN, CONFIDENCE_COLUMN]
        )
        assert len(frame) == 12
