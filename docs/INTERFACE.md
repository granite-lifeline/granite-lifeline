...
**Status:** Confirmed — 5-type anomaly_type enum (`electronic_throttle_tracking_fault` and `idle_speed_control_or_surge_degradation` removed by the Data Layer; `intake_air_temperature_sensor_or_heat_soak_fault` renamed to `intake_air_temperature_sensor_fault`, 2026-07-19); 2 types pending Model Layer support, with their §2.4 key-signal mappings and rationales supplied by the Data Layer on 2026-07-19; Section 1 retains the Data Layer's confirmed legacy `feature_dataset.csv` delivery record (returned 2026-07-08; `trip_id` request satisfied) and documents its planned replacement; Model Layer Story 3 hardening retained (`notes` field, null estimation placeholders, `accelerator_pedal_sensor` detection); Model Layer input requirements retained as §1.5 (2026-07-11); Data Layer `production_features.csv` contract updated on 2026-07-19 to retain 4 sample keys, 16 A-class context/raw fields, 24 B-class production features, and 2 provenance fields (46 columns total; production feature count remains 24), with implementation and real-file delivery pending scripts 00–41
...
>
> **Production handoff update (2026-07-19):** the replacement `production_features.csv` contract contains 46 ordered columns: 4 sample keys, 16 A-class context/raw fields, 24 B-class production features, and 2 provenance fields. The B-class production feature count remains 24. This confirms the replacement interface definition; scripts 00–41 and the real output file are still pending. The 2026-07-08 statements above remain the delivery record for the currently implemented legacy `feature_dataset.csv`.
