...
**Last verified on 2026-08-10** against Model repo HEAD `275fc85`, the live main-repo contract, and the latest pulled `gl-406-viva-slides-polish-0-1` base at `a3cf480`. Fresh Model test evidence: **209 passed, 16 skipped, three warnings**. If the pipeline changes again, re-check the Evaluation and TTM-configuration sections first.
...
9. **Batch mode and dashboard contract** — `--batch` sweeps every window of a trip and emits `{summary, windows}`, where `summary` is the highest-risk window in the *unchanged* single-window shape. The window list exposes whether evidence persists, but persistence is not a gate for the current diagnosis: `summary` still selects one maximum-risk window. Every run appends its windows to `risk_history.csv` [deduped on trip + window], which feeds the separate trip-level projection. Expected failures exit non-zero with a single `ERROR: <message>` line on stderr and no traceback.
...
We plant controlled synthetic changes in healthy data — because we applied them, we know the expected family. Fourteen perturbation settings at varying strength: engine-temperature offsets of +5, +10, +15 °C; airflow under-reads at 95%, 90%, 80%, 70% of true; and pedal changes as one-channel offsets of 2, 5, 10, 20 percentage points plus gain errors of 1.05, 1.10, 1.20. Each runs over 11 usable held-out stretches, giving 154 injected cases. A separate intake-pressure ×1.25 attribution control and healthy controls are excluded from that 154-positive denominator. Upfront limitation: this proves response to artificial changes, not real breakdowns. *[synthetic injection; precision/recall]*
