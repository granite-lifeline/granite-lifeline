...
- Runtime output follows Shared Conventions: S1 and S3 use verdict role; S2 uses support role. S3 range rule (TBD closed): any sample of `intake_temp` outside −40…215°C [19] → `triggered` (direction low → P0112, high → P0113); evaluated continuously; `not_evaluable` only when the signal is missing.
