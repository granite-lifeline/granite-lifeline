...
5. **Forecast** — the selected epoch-five fine-tuned model predicts 96 seconds of six readings from 512 seconds of context [Granite TTM, `ibm-granite/granite-timeseries-ttm-r2`; channels rpm/speed/coolant_temp/map/maf/tps].
6. **Gap measurement** — absolute predicted-versus-actual gap, averaged per signal and divided by that signal's baseline reference-range width so different readings are comparable [mean absolute residual; normalisation vs `REFERENCE_RANGES` span].
...
- **Configuration boundary:** the final runtime and formal evaluation use the same epoch-five artefact on TTM's `common_channel` path, with forecast-channel mixing disabled. Shared weights forecast each of the six signals independently; the backbone does **not** learn MAP--MAF or pedal-channel correlations. Those relationships are interpreted downstream through engineered physical evidence.
- **Why residuals detect anomalies:** the model learns plausible temporal continuation. If what happens next differs, prediction error becomes anomaly evidence. Quality-gated fine-tuning reduced held-out forecast MAE by 5.23%; whether that improves fault detection is a separate question answered by the synthetic campaign, not by MAE alone.
- **Where forecast quality and detection quality part company:** a lower prediction error improves the baseline the detector works from, but it does not automatically improve detection of every fault. Fine-tuning reduced overall error and improved five of six signals, yet MAF detection stayed near zero because that fault's signature was barely visible in the forecast error. Two connected measures, two different evaluation criteria — worth saying if a marker conflates them.
...
- **One car, one driver.** The baseline and alarm levels are calibrated on a single vehicle's data. No evidence they transfer to another car without re-calibration.
...
- **One baseline stretch still alarms at maximum risk.** In the calibration hold-out, one of three baseline segments scores 1.0, and no threshold at or below 1.0 removes it without disabling all detection. That's why the published alarm policy is marked provisional.
...
- **What we'd do differently:** obtain an externally labelled fault dataset from the start; design gradual, realistic fault injections instead of step changes; plan a second vehicle's data for transfer testing; and define an acceptance test requiring the evaluated model artefact to match the runtime artefact.
...
| fine-tuning | "extra training on this car's quality-gated unlabelled data" |
...
Test on an external dataset with mechanically verified fault labels; use the Data Layer's proxy decisions for integration evidence, not as a substitute for real ground truth; redesign the airflow evidence; design gradual, realistic fault injections; evaluate transfer on a second vehicle; and add model provenance to runtime outputs.

### Q13. Which model produced those evaluation numbers — the one used by the final pipeline?

**Yes.** The formal evaluation and the final integrated pipeline use the same committed epoch-five fine-tuned artefact. The released zero-shot checkpoint is retained only in the separate forecasting comparison. The production detector does not expose a runtime model override, and a regression test verifies that it loads the evaluated artefact.
