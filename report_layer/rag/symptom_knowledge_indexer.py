"""
ChromaDB Symptom Knowledge Indexer for Document-Level Chunking.

This module indexes the grounded knowledge YAML into ChromaDB using
document-level chunking. Each anomaly type is stored as a single merged
document containing description, causes, and all actions.

Task: GL-166 (Expand Knowledge Base and Four-Way Retrieval Comparison)
Project: Granite Lifeline MSc Project, University of Bristol
(IBM-sponsored)
"""
...
# Five current anomaly types from docs/INTERFACE.md v1.5.
...
def create_merged_document(
    ...
    # Merge all content in order: description, causes, actions
    causes_text = "\n".join(causes)
...
def main() -> None:
    ...
    print("=" * 79)
    print("Granite Lifeline - Symptom Knowledge Base Indexer")
    print("Task: GL-166 (Four-Way Retrieval Comparison)")
    print("=" * 79)
    print()
    ...
    try:
        ...
        print("  - 1 merged document per type (document-level chunking)")
        print("=" * 79)
