...
- **Failure projections now exist, but they are trend projections rather than calibrated mechanical-failure probabilities.** Story 8 is implemented and the current real sample carries non-null `estimated_cycles_to_failure` / `estimated_failure_probability` values. The honest limitation is that they are derived from risk-score history and deliberately-rising synthetic history, not labelled real failure times, so the report must present them as model projections rather than guaranteed remaining life.
...
> Story 8 is now implemented: when the Model Layer has enough rising risk history, it provides `estimated_failure_probability` and `estimated_cycles_to_failure`, and the Report Layer passes them through without inventing missing values. Honest limitation: the number is a projection from risk-score history, not an empirically calibrated probability that a real vehicle will fail by that trip count.
